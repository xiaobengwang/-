...
namespace Calculate
{
    ...
    public class Calculator
    {
        ...
        public static Queue<object> PreOrderToPostOrder(string expression) 
        {
            ...
            for (int i = 0, j; i < expression.Length; )
            {
                ...
                else
                {
                    ...
                    else
                    {
                        tempNum = "" + cur;
                        j = i;
                        if (j < expression.Length && (expression[j] == '.' || (expression[j] >= '0' && expression[j] <= '9')))
                        {
        ...
        public static double Calucate(string expression)
        {
            ...
            {
                throw new Exception("表达式格式不正确！", ex);
            }
        }
